...
function onAmountChanged(e) {
    ...
    {
        $(e).closest('tr').find('input')[2].value = 0.0;
    }
}
...
var pageManager = function () {
    ...
    var
        ...
        bindFormControls = function (d) {
            ...
            // expenses
            if (jsn) {
                var _options = $(jsn).map(function (i, v) { return $('<option data-IsVatable=s />').val(v.ExpenseID).text(v.ExpenseName); }).get();
                ...
                var detailData = (_id) ? jsn3 : jsn,
                    rows = $(detailData).map(function (i, v) {
                        return $('<tr><td data-expenseid="' + v.ExpenseID + '" data-inv-details-id="' + (v.InvoiceDetailsID ? v.InvoiceDetailsID : 0) + '" class="center">' + (i + 1) + '</td><td>' + v.ExpenseName + '</td>\
                             ...
                             <td><input onchange="onAmountChanged(this);" data-IsVatable="'+ v.IsVatable + '" data-expid="' + v.ExpenseID + '" type="number" value="' + numeral(v.Amount ? v.Amount : v.DefaultValue).format('0.0') + '" /></td>\
        ...
        ReadFromSettings = function () {
            var
                acName = 'Settings_Select', // function name
                DTO = { actionName: acName }; // set paramers for edit only.

            dataService.callAjax('Post', JSON.stringify(DTO), sUrl + 'GetDataDirect',
                GetVat, commonManger.errorException);
        },
        GetVat = function (d) {
            
            var xml = $.parseXML(d.d), jsn = $.xml2json(xml).list;
            selfInviceAddManager.VAT = parseFloat(jsn[1].Val);
        },
